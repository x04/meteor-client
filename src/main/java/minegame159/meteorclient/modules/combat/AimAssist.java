...
public class AimAssist extends ToggleModule {
    ...
    // General
    private final Setting<Double> range = sgGeneral.add(new DoubleSetting.Builder().name("range").description("Aim range.").defaultValue(5).min(0).build());
    private final Setting<List<EntityType<?>>> entities = sgGeneral.add(new EntityTypeListSetting.Builder().name("entities").description("Entities to aim at.").defaultValue(new ArrayList<>(0)).build());
    private final Setting<Boolean> friends = sgGeneral.add(new BoolSetting.Builder().name("friends").description("Aim at friends, useful only if attack players is on.").defaultValue(false).build());
    private final Setting<Boolean> ignoreWalls = sgGeneral.add(new BoolSetting.Builder().name("ignore-walls").description("Aim through walls.").defaultValue(false).build());
    ...
    private final Setting<Target> target = sgGeneral.add(new EnumSetting.Builder<Target>().name("target").description("Where to aim.").defaultValue(Target.Body).build());
    // Speed
    private final Setting<Boolean> speedInstant = sgSpeed.add(new BoolSetting.Builder().name("speed-instant").description("Instantly looks at the entity.").defaultValue(false).build());
    private final Setting<Double> speed = sgSpeed.add(new DoubleSetting.Builder().name("speed").description("How fast to aim at the entity.").defaultValue(5).min(0).build());
    ...
    private void setVec3dToTargetPoint(Vec3d vec3d, Entity entity) {
        switch (target.get()) {
            case Eyes:
                ((IVec3d) vec3d).set(entity.getX(), entity.getY() + entity.getEyeHeight(entity.getPose()), entity.getZ());
                break;
            case Body:
                ((IVec3d) vec3d).set(entity.getX(), entity.getY() + entity.getEyeHeight(entity.getPose()) / 2, entity.getZ());
                break;
            case Feet:
                ((IVec3d) vec3d).set(entity.getX(), entity.getY(), entity.getZ());
                break;
        }
    }
