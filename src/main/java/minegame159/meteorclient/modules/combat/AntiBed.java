...
public class AntiBed extends ToggleModule {
    ...
    private final Setting<Boolean> selfToggle = sgGeneral.add(new BoolSetting.Builder().name("self-toggle").description("Toggles this module when it's finished.").defaultValue(false).build());

    private final Setting<Boolean> autoCenter = sgGeneral.add(new BoolSetting.Builder().name("auto-center").description("Moves you to the center of blocks when needed.").defaultValue(true).build());
    ...
    @EventHandler private final Listener<TickEvent> onTick = new Listener<>(event -> {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (item == Items.STRING || block instanceof TrapdoorBlock || item == Items.COBWEB) {
                place(i);
                return;
                ...
                place(i);
                return;
            } else if (item instanceof BannerItem || item == Items.LEVER || item == Items.TORCH || item == Items.REDSTONE_TORCH || item instanceof SignItem || item == Items.TRIPWIRE_HOOK || block instanceof StoneButtonBlock || block instanceof WoodenButtonBlock) {
                place(i);
