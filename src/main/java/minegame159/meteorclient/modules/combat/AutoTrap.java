...
public class AutoTrap extends ToggleModule {
    ...
    private final Setting<topMode> topPlacement = sgGeneral.add(new EnumSetting.Builder<topMode>().name("top-mode").description("Which blocks to place on the top half of the target.").defaultValue(topMode.Full).build());
    private final Setting<bottomMode> bottomPlacement = sgGeneral.add(new EnumSetting.Builder<bottomMode>().name("bottom-mode").description("Which blocks to place on the bottom half of the target.").defaultValue(bottomMode.Single).build());
    ...
    @EventHandler private final Listener<TickEvent> onTick = new Listener<>(event -> {
        ...
        for (PlayerEntity player : mc.world.getPlayers()) {
            if (player == mc.player || !FriendManager.INSTANCE.attack(player)) {
                continue;
            }
            if (target == null) {
                target = player;
