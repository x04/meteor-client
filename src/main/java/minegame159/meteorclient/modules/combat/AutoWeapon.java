...
public class AutoWeapon extends ToggleModule {
    ...
    private final Setting<Weapon> weapon = sgGeneral.add(new EnumSetting.Builder<Weapon>().name("Weapon").description("Which weapon to use for AutoWeapon").defaultValue(Weapon.Sword).build());
    private final Setting<Integer> threshold = sgGeneral.add(new IntSetting.Builder().name("threshold").description("If the non-prefered weapon does this much damage more then it will chose that over the prefered").defaultValue(4).build());
    ...
    @EventHandler private final Listener<AttackEntityEvent> onAttack = new Listener<>(event -> {
    ...
    private int getBestWeapon() {
        ...
        for (int i = 0; i < 9; i++) {
            if (mc.player.inventory.getStack(i).getItem() instanceof SwordItem && (!antiBreak.get() || (mc.player.inventory.getStack(i).getMaxDamage() - mc.player.inventory.getStack(i).getDamage()) > 10)) {
        ...
        for (int i = 0; i < 9; i++) {
            if (mc.player.inventory.getStack(i).getItem() instanceof AxeItem && (!antiBreak.get() || (mc.player.inventory.getStack(i).getMaxDamage() - mc.player.inventory.getStack(i).getDamage()) > 10)) {
