...
public class Criticals extends ToggleModule {
    ...
    private final Setting<Mode> mode = sgGeneral.add(new EnumSetting.Builder<Mode>().name("mode").description("Mode.").defaultValue(Mode.Packet).build());
    ...
    private void doPacketMode() {
        ...
        if (sqrt(mc.player.getVelocity().x * mc.player.getVelocity().x + mc.player.getVelocity().z * mc.player.getVelocity().z) > 0.2f) {
            mc.player.setVelocity(sin(-yaw) * 0.2f, mc.player.getVelocity().y, cos(yaw) * 0.2f);
        }
