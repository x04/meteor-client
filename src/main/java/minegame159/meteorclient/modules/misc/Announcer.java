...
public class Announcer extends ToggleModule {
    ...
    private abstract class Feature implements Listenable {
        ...
        protected Feature(String name, String enabledName, String enabledDescription) {
            ...
            enabled = sg.add(new BoolSetting.Builder().name(enabledName).description(enabledDescription).defaultValue(true).onChanged(aBoolean -> {
    ...
    private class Moving extends Feature {
        private final Setting<String> msg = sg.add(new StringSetting.Builder().name("moving-msg").description("Moving message.").defaultValue("I just moved {dist} blocks!").build());

        private final Setting<Double> delay = sg.add(new DoubleSetting.Builder().name("moving-delay").description("Moving delay between messages in seconds.").defaultValue(10).sliderMax(60).build());

        private final Setting<Double> minDist = sg.add(new DoubleSetting.Builder().name("moving-min-dist").description("Moving minimum distance.").defaultValue(10).sliderMax(100).build());
    ...
    private class Mining extends Feature {
        private final Setting<String> msg = sg.add(new StringSetting.Builder().name("mining-msg").description("Mining message.").defaultValue("I just mined {count} {block}!").build());

        private Block lastBlock;
        private int count;
        private double notBrokenTimer;
        @EventHandler private final Listener<BreakBlockEvent> onBreakBlock = new Listener<>(event -> {
    ...
    private class Placing extends Feature {
        private final Setting<String> msg = sg.add(new StringSetting.Builder().name("placing-msg").description("Placing message.").defaultValue("I just placed {count} {block}!").build());

        private Block lastBlock;
        private int count;
        private double notPlacedTimer;
        @EventHandler private final Listener<PlaceBlockEvent> onPlaceBlock = new Listener<>(event -> {
    ...
    private class DropItems extends Feature {
        private final Setting<String> msg = sg.add(new StringSetting.Builder().name("drop-items-msg").description("Drop items message.").defaultValue("I just dropped {count} {item}!").build());

        private Item lastItem;
        private int count;
        private double notDroppedTimer;
        @EventHandler private final Listener<DropItemsEvent> onDropItems = new Listener<>(event -> {
    ...
    private class PickItems extends Feature {
        private final Setting<String> msg = sg.add(new StringSetting.Builder().name("pick-items-msg").description("Pick items message.").defaultValue("I just picked up {count} {item}!").build());

        private Item lastItem;
        private int count;
        private double notPickedUpTimer;
        @EventHandler private final Listener<PickItemsEvent> onPickItems = new Listener<>(event -> {
            if (lastItem != null && lastItem != event.itemStack.getItem()) {
                sendMsg();
            }
    ...
    private class OpenContainer extends Feature {
        private final Setting<String> msg = sg.add(new StringSetting.Builder().name("open-container-msg").description("Open container message.").defaultValue("I just opened {name}!").build());
