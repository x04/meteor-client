...
public class EntityLogger extends ToggleModule {
    ...
    private final Setting<Boolean> playerNames = sgGeneral.add(new BoolSetting.Builder().name("player-names").description("Show player names.").defaultValue(true).build());

    private final Setting<Boolean> friends = sgGeneral.add(new BoolSetting.Builder().name("friends").description("Log friends.").defaultValue(true).build());
    @EventHandler private final Listener<EntityAddedEvent> onEntityAdded = new Listener<>(event -> {
        ...
        if (entities.get().contains(event.entity.getType())) {
            if (event.entity instanceof PlayerEntity) {
                if (!friends.get() && FriendManager.INSTANCE.get((PlayerEntity) event.entity) != null) {
                    return;
                }
            }
            ...
            if (playerNames.get() && event.entity instanceof PlayerEntity) {
                ...
                name = event.entity.getType().getName().getString();
            }
