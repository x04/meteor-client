...
public class ItemByteSize extends ToggleModule {
    ...
    private final Setting<Boolean> useKbIfBigEnoughEnabled = sgUseKbIfBigEnough.add(new BoolSetting.Builder().name("use-kb-if-big-enough-enabled").description("Uses kilobytes instead of bytes if the item is larger than 1 kb.").defaultValue(true).build());
    private final Setting<Mode> mode = sgUseKbIfBigEnough.add(new EnumSetting.Builder<Mode>().name("mode").description("Standard 1 kb = 1000 b, True 1 kb = 1024 b.").defaultValue(Mode.True).build());
