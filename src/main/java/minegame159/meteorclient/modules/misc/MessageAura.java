...
public class MessageAura extends ToggleModule {
    ...
    private final Setting<String> message = sgGeneral.add(new StringSetting.Builder().name("message").description("The message sent to players.").defaultValue("Meteor on Crack!").build());
    private final Setting<Boolean> ignoreFriends = sgGeneral.add(new BoolSetting.Builder().name("ignore-friends").description("Doesn't send messages to friends.").defaultValue(false).build());
