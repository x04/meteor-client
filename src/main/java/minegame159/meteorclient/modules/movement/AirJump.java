...
public class AirJump extends ToggleModule {
    ...
    private final Setting<Boolean> maintainY = sgGeneral.add(new BoolSetting.Builder().name("maintain-level").description("Maintains your Y level").defaultValue(false).build());
    private final Setting<Boolean> onHold = sgGeneral.add(new BoolSetting.Builder().name("on-hold").description("Whether to jump when you hold jump as well as when you press.").defaultValue(true).build());
    private int level = 0;
