...
public class InvMove extends ToggleModule {
    ...
    private final Setting<Boolean> sneak = sgGeneral.add(new BoolSetting.Builder().name("sneak").description("Allows you to sneak.").defaultValue(true).build());

    private final Setting<Boolean> jump = sgGeneral.add(new BoolSetting.Builder().name("jump").description("Allows you to jump.").defaultValue(true).build());

    private final Setting<Boolean> sprint = sgGeneral.add(new BoolSetting.Builder().name("sprint").description("Allows you to sprint.").defaultValue(true).build());

    private final Setting<Boolean> arrowsRotate = sgGeneral.add(new BoolSetting.Builder().name("arrows-rotate").description("Allows you to use arrow keys to rotate.").defaultValue(true).build());
    ...
    public void tick() {
        ...
        if (arrowsRotate.get()) {
            if (Input.isPressed(GLFW.GLFW_KEY_RIGHT)) {
                mc.player.yaw += rotateSpeed.get();
            }
            if (Input.isPressed(GLFW.GLFW_KEY_LEFT)) {
                mc.player.yaw -= rotateSpeed.get();
            }
            if (Input.isPressed(GLFW.GLFW_KEY_UP)) {
                mc.player.pitch -= rotateSpeed.get();
            }
            if (Input.isPressed(GLFW.GLFW_KEY_DOWN)) {
                mc.player.pitch += rotateSpeed.get();
            }
