...
public class NoFall extends ToggleModule {
    ...
    private final Setting<Boolean> elytra = sgGeneral.add(new BoolSetting.Builder().name("elytra compatibility").description("Stops this from working when using elytra.").defaultValue(true).build());
    ...
    @EventHandler private final Listener<SendPacketEvent> onSendPacket = new Listener<>(event -> {
        if (event.packet instanceof PlayerMoveC2SPacket) {
            if (elytra.get() && (mc.player.getEquippedStack(EquipmentSlot.CHEST).getItem() == Items.ELYTRA && mc.options.keyJump.isPressed() || mc.player.isFallFlying())) {
                ...
                ((IPlayerMoveC2SPacket) event.packet).setOnGround(true);
            } else if ((placeMode.get() == PlaceMode.BeforeDamage && mc.player.fallDistance > 2) || (placeMode.get() == PlaceMode.BeforeDeath && ((mc.player.getHealth() + mc.player.getAbsorptionAmount()) < mc.player.fallDistance))) {
                int slot = -1;
                int preSlot;
                for (int i = 0; i < 9; i++) {
