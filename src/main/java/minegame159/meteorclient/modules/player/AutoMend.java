...
public class AutoMend extends ToggleModule {
    ...
    private final Setting<Boolean> swords = sgGeneral.add(new BoolSetting.Builder().name("swords").description("Move swords.").defaultValue(true).build());

    private final Setting<Boolean> armourSlots = sgGeneral.add(new BoolSetting.Builder().name("use-armour-slots").description("Whether to use armour slots to mend items faster").defaultValue(true).build());
    ...
    @EventHandler private final Listener<TickEvent> onTick = new Listener<>(event -> {
        ...
        if (mc.player.getOffHandStack().isEmpty()) {
            replaceItem(true);
        } else if (!mc.player.getOffHandStack().isDamaged()) {
            replaceItem(false);
        } else if (EnchantmentHelper.getLevel(Enchantments.MENDING, mc.player.getOffHandStack()) == 0) {
            replaceItem(false);
        }
        ...
        if (armourSlots.get()) {
            ...
            for (int i = 5; i < 9; i++) {
                if (mc.player.inventory.getStack(39 - (i - 5)).isEmpty()) {
                    replaceArmour(i, true);
                } else if (!mc.player.inventory.getStack(39 - (i - 5)).isDamaged()) {
    ...
    private void replaceItem(boolean offhandEmpty) {
        for (int i = 0; i < mc.player.inventory.main.size(); i++) {
            ...
            if (EnchantmentHelper.getLevel(Enchantments.MENDING, itemStack) == 0 || !itemStack.isDamaged()) {
                continue;
            }
            if (!swords.get() && itemStack.getItem() instanceof SwordItem) {
                continue;
            }
    ...
    private boolean checkSlot(ItemStack itemStack, int slot) {
        ...
        if (slot == 5 && ((ArmorItem) itemStack.getItem()).getSlotType() == EquipmentSlot.HEAD) {
            correct = true;
        } else if (slot == 6 && ((ArmorItem) itemStack.getItem()).getSlotType() == EquipmentSlot.CHEST) {
            correct = true;
        } else if (slot == 7 && ((ArmorItem) itemStack.getItem()).getSlotType() == EquipmentSlot.LEGS) {
            correct = true;
        } else if (slot == 8 && ((ArmorItem) itemStack.getItem()).getSlotType() == EquipmentSlot.FEET) {
            correct = true;
        }
        return correct;
    }
    ...
    private void replaceArmour(int slot, boolean empty) {
        for (int i = 0; i < mc.player.inventory.main.size(); i++) {
            ...
            if (!checkSlot(mc.player.inventory.getStack(i), slot)) {
                continue;
            }
            if (EnchantmentHelper.getLevel(Enchantments.MENDING, itemStack) == 0 || !itemStack.isDamaged()) {
                continue;
