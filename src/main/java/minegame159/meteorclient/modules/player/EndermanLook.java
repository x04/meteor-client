...
public class EndermanLook extends ToggleModule {
    ...
    @EventHandler private final Listener<TickEvent> onTick = new Listener<>(event -> {
        if (lookMode.get() == Mode.LookAway) {
            ...
            PlayerMoveC2SPacket.LookOnly packet = new PlayerMoveC2SPacket.LookOnly(mc.player.yaw, 90.0f, mc.player.isOnGround());
            ...
            Streams.stream(mc.world.getEntities()).filter(entity -> entity instanceof EndermanEntity).filter(entity -> !((EndermanEntity) entity).isAngry()).filter(Entity::isAlive).filter(entity -> mc.player.canSee(entity)).findFirst().ifPresent(tempEntity -> enderman = ((EndermanEntity) tempEntity));
        }
    });
    ...
    private boolean shouldLook() {
        return Streams.stream(mc.world.getEntities()).filter(entity -> entity instanceof EndermanEntity).filter(Entity::isAlive).anyMatch(this::angleCheck);
    }
