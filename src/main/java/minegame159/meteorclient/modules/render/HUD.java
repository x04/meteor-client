...
public class HUD extends ToggleModule {
    ...
    // Bottom Right
    public final Setting<Boolean> potionTimers = sgBottomRight.add(new BoolSetting.Builder().name("potion-timers").description("Display potion timers and hide minecraft default potion icons.").defaultValue(true).build());
    private final SettingGroup sgPlayerModel = settings.createGroup("Player Model");
    public final Setting<Color> bgColor = sgPlayerModel.add(new ColorSetting.Builder().name("background-color").description("Background color.").defaultValue(new Color(0, 0, 0, 30)).build());
    // Armor
    private final Setting<Boolean> armorEnabled = sgArmor.add(new BoolSetting.Builder().name("armor-enabled").description("Armor HUD.").defaultValue(true).build());
    private final Setting<DurabilityType> armorDurability = sgArmor.add(new EnumSetting.Builder<DurabilityType>().name("armor-durability").description("Displays armor durability on top of hotbar").defaultValue(DurabilityType.Default).build());
    private final Setting<Double> scale = sgArmor.add(new DoubleSetting.Builder().name("scale").description("The scale of the numbers over the armor").min(0.5).max(1).sliderMax(1).defaultValue(0.8).build());
    private final Setting<Boolean> armorWarning = sgArmor.add(new BoolSetting.Builder().name("durability-warner").description("Warns you if your armor is about to break.").defaultValue(true).build());
    private final Setting<Integer> warningDurability = sgArmor.add(new IntSetting.Builder().name("warn-durability").description("The durability you are warned at").defaultValue(30).min(1).max(360).sliderMax(100).build());
    private final Setting<Boolean> notificationSettings = sgArmor.add(new BoolSetting.Builder().name("chat-notifications").description("Sends messages in chat rather than in the corner of the screen.").defaultValue(false).build());
    // Top Center
    private final Setting<Boolean> serverLagNotifier = sgTopCenter.add(new BoolSetting.Builder().name("lag-notifier").description("Time since last tick.").defaultValue(true).build());
    private final Setting<Boolean> userWelcome = sgTopCenter.add(new BoolSetting.Builder().name("user-welcome").description("Welcome message.").defaultValue(true).build());
    // Top Left
    private final Setting<Boolean> waterMark = sgTopLeft.add(new BoolSetting.Builder().name("water-mark").description("Water mark.").defaultValue(true).build());
    private final Setting<Boolean> fps = sgTopLeft.add(new BoolSetting.Builder().name("fps").description("Display fps.").defaultValue(true).build());
    private final Setting<Boolean> ping = sgTopLeft.add(new BoolSetting.Builder().name("ping").description("Display ping.").defaultValue(true).build());
    private final Setting<Boolean> tps = sgTopLeft.add(new BoolSetting.Builder().name("tps").description("Display tps.").defaultValue(true).build());
    private final Setting<Boolean> speed = sgTopLeft.add(new BoolSetting.Builder().name("speed").description("Display speed in blocks per second.").defaultValue(true).build());
    private final Setting<Boolean> biome = sgTopLeft.add(new BoolSetting.Builder().name("biome").description("Displays biome you are in.").defaultValue(true).build());
    private final Setting<Boolean> time = sgTopLeft.add(new BoolSetting.Builder().name("time").description("Displays ingame time in ticks.").defaultValue(true).build());
    private final Setting<Boolean> durability = sgTopLeft.add(new BoolSetting.Builder().name("durability").description("Durability of the time in your main hand.").defaultValue(true).build());
    private final Setting<Boolean> breakingBlock = sgTopLeft.add(new BoolSetting.Builder().name("breaking-block").description("Displays the percentage how much you have broken the block.").defaultValue(true).build());
    private final Setting<Boolean> lookingAt = sgTopLeft.add(new BoolSetting.Builder().name("looking-at").description("Displays block or entity you are looking at.").defaultValue(true).build());
    private final Setting<Boolean> combatInfo = sgTopLeft.add(new BoolSetting.Builder().name("combat-info").description("Shows which combat modules you have on.").defaultValue(false).build());

    // Minimap
    private final Setting<Boolean> minimapEnabled = sgMinimap.add(new BoolSetting.Builder().name("minimap-enabled").description("Minimap HUD.").defaultValue(true).build());

    private final Setting<Double> mmScale = sgMinimap.add(new DoubleSetting.Builder().name("minimap-scale").description("Scale.").defaultValue(1).min(0).sliderMax(2).build());

    private final Setting<Color> mmBackground = sgMinimap.add(new ColorSetting.Builder().name("minimap-background").description("Minimap background color.").defaultValue(new Color(25, 25, 25, 175)).build());

    private final Setting<Color> mmPlayer = sgMinimap.add(new ColorSetting.Builder().name("minimap-player").description("Minimap player color.").defaultValue(new Color(225, 225, 225, 225)).build());

    private final Setting<Color> mmAnimal = sgMinimap.add(new ColorSetting.Builder().name("minimap-animal").description("Minimap animal color.").defaultValue(new Color(25, 225, 25, 225)).build());

    private final Setting<Color> mmMob = sgMinimap.add(new ColorSetting.Builder().name("minimap-mob").description("Minimap mob color.").defaultValue(new Color(225, 25, 25, 225)).build());

    // Top Right
    private final Setting<Boolean> activeModules = sgTopRight.add(new BoolSetting.Builder().name("active-modules").description("Display active modules.").defaultValue(true).build());
    private final Setting<Boolean> position = sgBottomRight.add(new BoolSetting.Builder().name("position").description("Display your position.").defaultValue(true).build());
    private final Setting<Boolean> rotation = sgBottomRight.add(new BoolSetting.Builder().name("rotation").description("Display your rotation.").defaultValue(true).build());
    // Player Model
    private final Setting<Boolean> playerModel = sgPlayerModel.add(new BoolSetting.Builder().name("Player Model").description("Render mini player model.").defaultValue(true).build());
    private final Setting<Boolean> background = sgPlayerModel.add(new BoolSetting.Builder().name("render-background").description("Render a background behind the player model.").defaultValue(true).build());
    private final Setting<AlignmentX> xAlignment = sgPlayerModel.add(new EnumSetting.Builder<AlignmentX>().name("x-alignment").description("X alignment.").defaultValue(AlignmentX.Left).build());
    private final Setting<Integer> xOffset = sgPlayerModel.add(new IntSetting.Builder().name("x-offset").description("X offset.").defaultValue(3).sliderMax(200).build());
    private final Setting<AlignmentY> yAlignment = sgPlayerModel.add(new EnumSetting.Builder<AlignmentY>().name("y-alignment").description("Y alignment.").defaultValue(AlignmentY.Bottom).build());
    private final Setting<Integer> yOffset = sgPlayerModel.add(new IntSetting.Builder().name("y-offset").description("Y offset.").defaultValue(3).sliderMax(200).build());
    ...
    @EventHandler private final Listener<ActiveModulesChangedEvent> activeModulesChangedEventListener = new Listener<>(event -> recalculateActiveModules());
    @EventHandler private final Listener<ModuleVisibilityChangedEvent> onModuleVisibilityChanged = new Listener<>(event -> {
        if (event.module.isActive()) {
    ...
    @EventHandler private final Listener<EntityRemovedEvent> onEntityRemoved = new Listener<>(event -> {
        if (entities.get()) {
            if (!isValidEntity(event.entity)) {
                return;
            }
    ...
    @EventHandler private final Listener<TickEvent> onTick = new Listener<>(event -> {
        ...
        if (armorWarning.get()) {
            for (int i = mc.player.inventory.armor.size() - 1; i >= 0; i--) {
                ...
                } else if ((itemStack.getMaxDamage() - itemStack.getDamage()) > warningDurability.get()) {
    ...
    private String getEntityName(Entity entity) {
        ...
        if (separateSheepsByColor.get() && entity instanceof SheepEntity) {
            return StringUtils.capitalize(((SheepEntity) entity).getColor().getName().replace('_', ' ')) + " - " + name;
        }
        return name;
    }
    ...
    private void renderTopCenter(Render2DEvent event) {
...
*/
    ...
    private void renderTopLeft(Render2DEvent event) {
        ...
        if (minimapEnabled.get()) {
            ...
            for (Entity entity : mc.world.getEntities()) {
                ...
                if (entity.getEntityId() == mc.player.getEntityId()) {
                    renderMMTriangle(x2 - 2.5, z2 - 2.5, 5, mc.player.yaw, color);
                } else {
                    renderMMQuad(x2 - 1, z2 - 1, 2, 2, color);
                }
            }
        ...
        if (durability.get()) {
            ...
            if (!mc.player.getMainHandStack().isEmpty() && mc.player.getMainHandStack().isDamageable()) {
                amount = mc.player.getMainHandStack().getMaxDamage() - mc.player.getMainHandStack().getDamage();
            }
        ...
        if (lookingAt.get()) {
            ...
            if (mc.crosshairTarget.getType() == HitResult.Type.BLOCK) {
                text = mc.world.getBlockState(((BlockHitResult) mc.crosshairTarget).getBlockPos()).getBlock().getName().getString();
            } else if (mc.crosshairTarget.getType() == HitResult.Type.ENTITY) {
                text = ((EntityHitResult) mc.crosshairTarget).getEntity().getDisplayName().getString();
            }
    ...
    public enum DurabilityType {
        None, Default, Numbers, Percentage
    }
    ...
    private static class EntityInfo {
        ...
        public void increment(Entity entity) {
            if (entity instanceof ItemEntity) {
                count += ((ItemEntity) entity).getStack().getCount();
            } else {
                count++;
            }
